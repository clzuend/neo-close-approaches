...
def write_to_csv(results, filename):
    ...
    :param filename: A Path-like object pointing to where the data gets saved.
    """
    ...
    with open(filename, 'w') as file:
        ...
        for approach in results:
            csv_writer.writerow(approach.serialize() |
...
def write_to_json(results, filename):
    ...
    json_data = []
    for approach in results:
        json_data.append(approach.serialize() |
                         {'neo': approach.neo.serialize()})

    with open(filename, 'w') as file:
        json.dump(json_data, file, indent=4, allow_nan=True)
