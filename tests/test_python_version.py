"""Check that the Python version is at least up to a minimum threshold of 3.9.

The instructions explicitly invoke each command using `python3` on the command
line, but a student's local setup might not default to using Python 3.9+, which
...
Other modules in this project aggressively assume Python 3.9+, so this unit test
...
class TestPythonVersion(unittest.TestCase):
    ...
    def test_python_version_is_at_least_3_9(self):
    ...
    This project requires Python 3.9+. You're currently using Python {}.{}.{}.
    ...
    some Python3.9+ version if you have a custom setup).
